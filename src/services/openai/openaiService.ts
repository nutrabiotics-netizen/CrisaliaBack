...
IMPORTANTE: NUNCA incluyas identificadores técnicos de campo (como s01, s19, s22, s03, s04 u otros códigos alfanuméricos internos). Usa únicamente términos clínicos. Si un dato no está disponible en el interrogatorio, escribe "No reportado".`
  ...
  private construirPrompt(respuestas: Record<string, any>, contextoAgent?: { disfuncionesAgent?: any[]; notaMedico?: string; ordenAbordaje?: any[] }): string {
    ...
    const prompt = `Construye la Historia Clínica a partir del interrogatorio de Medicina Funcional.${bloqueAgent}
...
MOTIVO_CONSULTA:
Motivo principal: [texto]
Tiempo de evolución: [texto]
Síntoma o situación que genera la consulta: [texto]

ENFERMEDAD_ACTUAL:
Inicio: [texto]
Forma de aparición: [texto]
Evolución: [texto]
Síntomas asociados: [texto]
Factores desencadenantes: [texto]
Factores que mejoran o empeoran: [texto]
Tratamientos realizados: [texto]
Medicamentos utilizados: [texto]
Exámenes previos: [texto]
Resultados relevantes: [texto]
Consultas anteriores por el mismo motivo: [texto]
Estado actual: [texto]

ANTECEDENTES:
Patológicos: [texto]
Farmacológicos: [texto]
Quirúrgicos: [texto]
Alérgicos: [texto]
Familiares: [texto]
Ginecológicos: [texto o "No aplica" si es masculino]
Tóxicos: [texto]
...
OBSERVACIONES:
[incoherencias clínicas reales únicamente, o exactamente "Ninguna" si no las hay]`;
